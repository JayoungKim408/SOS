...
def create_model_discriminator(config):
  """Create the score model."""
  model = get_model('discriminator')(config)
...
def calc_gradient_penalty(netD, real_data, fake_data, lambda_=10):
    alpha = torch.rand(real_data.size(0), 1, 1).to(real_data)
    ...
    gradient_penalty = (
        (gradients.view(-1, real_data.size(1)).norm(2, dim=1) - 1) ** 2).mean() * lambda_
