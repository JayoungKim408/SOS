...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, truncated=False, train=False):
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, train=False):
  ...
  output = predictor_obj.update_fn(x, t)
  return output
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
                   ...
                   denoise=True, eps=1e-3, device='cuda', truncated=True, train=False):
  ...
  def pc_sampler(model, z=None, sample_size=None):
    ...
    for i in range(sde.N):
      t = timesteps[i]
      vec_t = torch.ones(size, device=t.device) * t
